...
def test_bounding_box_validation():
    ...
    assert bbox.xmax == 350

    # Invalid bounding box (out of range > 1000)
    with pytest.raises(Exception):
        BoundingBox(page=1, ymin=-10, xmin=0, ymax=1050, xmax=500)


def test_form26_payload_structure():
    ...
    assert payload.candidate.name == "Rajesh Kumar"
    assert len(payload.part_a_movable_items) == 2
    assert payload.criminal_cases[0].is_serious_category is False


def test_forensic_audit_result_logic():
    ...
    assert audit.has_arithmetic_discrepancy is True
    assert audit.delta_movable == 200000.0
    assert audit.wealth_discrepancy_ratio == 2.4
